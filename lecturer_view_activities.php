...
<body>
  ...
  <section class="marksheet-container">
    ...
    <?php if (!empty($selected_activity) && $students->num_rows > 0): ?>
      <form method="POST" action="update_activity_status.php">
    ...
    <div class="table-cell">
      <div style="display: flex; gap: 30px; align-items: center;">
        ...
        <label style="font-weight: bold; color: #28a745;">
          <input type="radio" 
                name="participating[<?php echo $row['student_id']; ?>]" 
                value="1"
                <?php echo $participation === "1" || $participation === 1 ? 'checked' : ''; ?>>
          Yes
        </label>
        ...
        <label style="font-weight: bold; color: #dc3545;">
          <input type="radio" 
                name="participating[<?php echo $row['student_id']; ?>]" 
                value="0"
                <?php echo $participation === "0" || $participation === 0 ? 'checked' : ''; ?>>
          No
        </label>
      </div>
    </div>
...
<?php endwhile; ?>
      ...
      <button type="submit" class="register-btn" style="margin: 25px auto 0; display: block; width: fit-content; padding: 10px 25px;">
        Save Participation
      </button>
  ...
  <script>
  document.querySelector("form[action='update_activity_status.php']").addEventListener("submit", function(e) {
